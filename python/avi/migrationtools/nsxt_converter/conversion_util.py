...
class NsxtConvUtil(MigrationUtil):
    ...
    def get_conv_status(self, skipped, indirect_list, ignore_dict, nsxt_object,
        ...
        :param nsxt_object: Currant NSXT object
        ...
        for key in ignore_dict.keys():
            nsxt_val = nsxt_object.get(key)
            default_val = ignore_dict.get(key)
            if key in skipped and nsxt_val == default_val:
    ...
    def get_avi_pool_down_action(self, action):
        """
        Maps Pool down action from NsxT config to Avi Config
        :param action: NsxT action string
    ...
    def get_cc_algo_val(self, cc_algo):
        ...
        :param cc_algo: NsxT algorithm value
    ...
    def get_vs_ssl_profiles(self, profiles, avi_config, prefix,
                            merge_object_mapping, sys_dict, nsxt_config):
        """
        Searches for profile refs in converted profile config if not found
        creates default profiles
        :param profiles: profiles in nsxt config assigned to VS
        ...
        """
        # nsxt_profiles = nsxt_config.get("profile", {})
    ...
    def get_service_obj(self, destination, avi_config, enable_ssl,
        ...
        """
        Checks port overlapping scenario for port value 0 in nsxt config
    ...
    def add_ssl_to_pool(self, avi_pool_list, pool_ref, pool_ssl_profiles,
                        tenant='admin'):
        """
        nsxt serverside SSL need to be added to pool if VS contains serverside SSL
    ...
    def update_pool_for_persist(self, avi_pool_list, pool_ref, persist_profile,
                                ...
                                app_prof_type):
        """
        Updates pool for persistence profile assigned in nsxt VS config
    ...
    def get_snat_list_for_vs(self, snat_pool):
        """
        Converts the nsxt snat pool config object to Avi snat list
        :param snat_pool: nsxt snat pool config
    ...
    def get_vrf_context_ref(self, nsxt_entity_mem, vrf_config, entity_string,
                            entity_name, cloud):
        """
        Searches for vrf context refs in converted pool config
        :param nsxt_entity_mem: nsxt entity or object like pool
        :param vrf_config: converted vrf config
        :param entity_string: entity string
        :param entity_name: name of nsxt entity
        :param cloud: name of the cloud
        :return: returns list of vrf refs assigned to entity in avi config
        """
        vrf_ref = None
        nsxt_entity_mem = ':' in nsxt_entity_mem and nsxt_entity_mem.split(':')[0] \
                        or nsxt_entity_mem if nsxt_entity_mem else None
        vrf = 'vrf-' + nsxt_entity_mem.split('%')[1] \
            if nsxt_entity_mem and '%' in nsxt_entity_mem else None
    ...
    def net_to_static_route(self, nsxt_config, avi_config):
        """
        This method converts the net route to static routes and updates the
        VrfContext objects
        :param nsxt_config: parsed nsxt config
        :param avi_config: converted config in avi
        :return:
        """
        net_config = nsxt_config.get('route', {})
    ...
    def update_with_default_profile(self, profile_type, profile,
        ...
        """
        Profiles can have inheritance used by attribute defaults-from in nsxt
        ...
        :param profile_config: NsxT profile config dict
